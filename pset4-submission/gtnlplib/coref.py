...
def get_markables_for_entity(markables, entity):
    ...
    :rtype: list
    '''
    return [" ".join(x.string) for x in markables if x.entity == entity]
...
def get_tp(pred_ant, markables):
    ...
    for i in range(len(pred_ant)):
        for entity in entities:
            if pred_ant[i] in entity:
                tp.append(True if i in entity and pred_ant[i] < i else False)
                break
...
def get_fp(pred_ant, markables):
    ...
    for i in range(len(pred_ant)):
        for entity in entities:
            if pred_ant[i] in entity:
                tp.append(True if i not in entity and pred_ant[i] < i else False)
                break
...
def get_fn(pred_ant, markables):
    ...
    '''
    entities = get_entities(markables)
    true_ant = get_true_antecedents(markables)
    tp = []
    for i in range(len(true_ant)):
        for entity in entities:
            if i in entity:
                tp.append(True if true_ant[i] in entity and true_ant[i] < i and (pred_ant[i] == i or pred_ant[i] not in entity) else False)
                break
