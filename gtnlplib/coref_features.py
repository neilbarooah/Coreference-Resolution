...
def distance_features(x,a,i,
    ...
    f = defaultdict(float)
    if a != i:
        mention = min(i - a, max_mention_distance)
        token = min(x[i].start_token - x[a].end_token, max_token_distance)
        f['mention-distance-' + str(mention)] = 1
        f['token-distance-' + str(token)] = 1
    return f
    ...
    def f_out(x, a, i):
        ...
        for feat_func in feat_func_list:
            ...
            for feature in features:
                union_feats[feature] = features[feature]
        return union_feats
    return f_out
